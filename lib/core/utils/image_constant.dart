class ImageConstant {
  static String imgFluenteye20filled =
      'assets/images/img_fluenteye20filled.svg';

  static String imgArrowleft = 'assets/images/img_arrowleft.svg';

  static String imgPeek2 = 'assets/images/img_peek2.png';

  static String imgWelcome = 'assets/images/img_welcome.png';

  static String imgframe = 'assets/images/frame.png';

  static String imgImage432x375 = 'assets/images/img_image4_32x375.png';

  static String imageNotFound = 'assets/images/image_not_found.png';

//

  static String imgStar4 = 'assets/images/img_star4.svg';

  static String imgMenudesplegble = 'assets/images/img_menudesplegble.svg';

  static String imgStar2 = 'assets/images/img_star2.svg';

  static String imgPeek299x96 = 'assets/images/img_peek2_99x96.png';

  static String imgFrame = 'assets/images/img_frame.png';

  static String imgCanelita = 'assets/images/img_canelita.png';

  static String imgBlackground = 'assets/images/img_blackground.png';

  static String imgImgframe = 'assets/images/img_imgframe.png';

  static String imgPlay = 'assets/images/img_play.svg';

  static String imgStar418x16 = 'assets/images/img_star4_18x16.svg';

  static String imgRectangle200 = 'assets/images/img_rectangle200.png';

  static String imgHuellita = 'assets/images/img_huellita.png';

  static String imgEllipse1 = 'assets/images/img_ellipse1.png';

  static String imgEllipse6 = 'assets/images/img_ellipse6.png';

  static String imgStar218x16 = 'assets/images/img_star2_18x16.svg';

  static String imgUltimospaseos = 'assets/images/img_ultimospaseos.png';

  static String imgStar3 = 'assets/images/img_star3.svg';

  static String imgBackground = 'assets/images/img_background.png';

  static String imgStar118x16 = 'assets/images/img_star1_18x16.svg';

  static String imgStar1 = 'assets/images/img_star1.svg';

  static String imgImage4 = 'assets/images/img_image4.png';

  static String imgStar318x16 = 'assets/images/img_star3_18x16.svg';

  static String imgCasa = 'assets/images/img_casa.png';

  static String imgPeek4 = 'assets/images/img_peek4.png';

  static String imgPerro = 'assets/images/img_perro.png';

  static String imgPexelssamsonkatt5257603 =
      'assets/images/img_pexelssamsonkatt5257603.png';

  static String imgUsuario = 'assets/images/img_usuario.png';

  static String imgMaskgroup = 'assets/images/img_maskgroup.png';

  static String imgMaskgroup39x39 = 'assets/images/img_maskgroup_39x39.png';

  static String imgFondoverde = 'assets/images/img_fondoverde.png';

  static String imgCachorro1 = 'assets/images/img_cachorro1.png';

  static String imgPaseador = 'assets/images/img_paseador.png';

  static String imgduenio = 'assets/images/img_duenio.png';
}
